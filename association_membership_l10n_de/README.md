...
- Annual receipts (type "Annual") print as **Sammelbestätigung**: total amount, period, the statement that no further receipts were or will be issued for the contained amounts, and an **Anlage** listing each contribution (date, Mitgliedsbeitrag, waiver "nein", amount). The rows come from the contributions linked to the receipt.
...
- Only membership contributions appear in the Anlage. Donations from OCA `donation` are not listed yet.
